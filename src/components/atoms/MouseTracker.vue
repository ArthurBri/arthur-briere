...
onMounted(() => {
  setInterval(() => {
    if (mouseTracker.value?.style) {
      mouseTracker.value.style.left = lerp(mouseTracker.value.style.left.replace('px', '') || 0, mouseX - 12 + scrollX, 0.3) + 'px'
      mouseTracker.value.style.top = lerp(mouseTracker.value.style.top.replace('px', '') || 0, mouseY - 12 + scrollY, 0.3) + 'px'
    }
  }, 40)
})
